...
第一次阅读本项目后，Agent 应使用渐进式引导，不要一次抛出完整流程。

第一轮只解决一个问题：确定产品目录。

```text
下一步只做一件事：请提供要接入的产品目录。
```
...
初始化完成后，第三轮只要求用户提供第一份产品资料，建议优先提供 PRD 或产品截图。用户可以通过三种方式补资料：
...
初始化完成后的默认回复不要展示完整资料目录。只有用户明确询问资料放哪里，或执行资料完整度检查时，才展开完整目录和资料清单。
