...
2. mobile_take_screenshot → 确认 WDA 工作且屏幕未锁定
...
**截图要求**：每个页面、每步操作都必须有对应截图，文件名规范为 `{序号}-{页面/步骤名}.png`。speckit 文档中通过 `![描述](screenshots/xx.png)` 引用截图，确保下游 vibe coding 有精确的视觉参照。
...
2. 如果存在：
   a. 读取 exploration-state.json → 确定上次停在哪个阶段
   b. 读取 feature-checklist.md → 确定哪些功能还没覆盖
   c. 扫描 screenshots/ → 确认已有截图
   d. 重新连接 WDA（可能需要重启）
...
**Level 3 — App 内功能目录**（关键新增）：

```
Step 7: 主动寻找 App 内部的"全部功能"/"更多工具"入口
    → 截图完整功能列表（可能需要滚动多次）
    → 用 App 内功能目录更新 feature-checklist（新增 Phase 1 未发现的功能）
    → 新增功能的 source 标记为 "in_app"
Step 8: 如遇到引导弹窗/新手引导/功能推广，截图并提取提到的功能卖点
    → 同样更新 feature-checklist
```

**Level 2/3 结束后**：对照 feature-checklist，确认每个功能至少有一张入口截图。未覆盖的功能立即补截图。
...
- 用户流程 → 从 Phase 2b 端到端流程直接转换为场景叙事，每步引用截图（`confirmed`）
...
| WDA 进程断开（会话切换/超时） | 所有 MCP tool 失效 | 检测到 WDA 断开后重新 `xcodebuild test-without-building` + `ios forward 8100 8100` |
